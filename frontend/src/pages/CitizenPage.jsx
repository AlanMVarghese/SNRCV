...
//   useEffect(() => {
//     if (user.volunteerDetails?.isAccepted) {
//       setVolunteerDetails(user.volunteerDetails);
//     }
//   }, [user]);
...
//           animate={{ opacity: 1, y: 0 }}
//           transition={{ duration: 0.5 }}
//           className="max-w-md w-full bg-blue-300 bg-opacity-50 backdrop-filter backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden mt-12"
//         >
//           <div className="p-8">
...
//           <div className="space-y-6">
//             <motion.div className="p-4 bg-white rounded-lg border">
//               <h3 className="text-xl font-semibold text-blue-700 mb-3">Current Status</h3>
//               {volunteerDetails?.isAccepted ? (
//                 <>
//                   <p className="text-black">A volunteer has accepted your request.</p>
//                   <p className="text-black">Volunteer Name: {volunteerDetails.name}</p>
//                   <p className="text-black">Contact No: {volunteerDetails.contactno}</p>
//                 </>
//               ) : (
...
const CitizenPage = () => {
  ...
  const [additional, setAdditional] = useState("");
  const [location, setLocation] = useState("");
  const { user, help, markHelpCompleted, error, isLoading, signout } =
    useAuthStore();
            ...
            >
              ...
              {user.volunteerDetails?.isAccepted ? (
                <>
                  ...
                  <p className="text-black">
                    Volunteer Name: {user.volunteerDetails.name}
                  </p>
                  <p className="text-black">
                    Contact No: {user.volunteerDetails.contactno}
                  </p>
                  <motion.button
                    onClick={handleMarkHelpCompleted}
                    className="mt-4 w-full py-2 bg-green-600 text-white font-bold rounded-lg shadow-md hover:bg-green-700"
